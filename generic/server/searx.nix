...
in
{
  ...
  systemd.services.searx-init.script = lib.mkForce ''
    ...
    meilisearch_masterKey="$(< ${config.age.secrets.meilisearch-masterKey.path})"
    ...
    meilisearch_masterKey_escaped="$(printf '%s' "$meilisearch_masterKey" | sed -e 's/[\/&]/\\&/g')"
    ...
    sed \
      ...
      -e "s/@MEILISEARCH_MASTERKEY@/$meilisearch_masterKey_escaped/g" \
